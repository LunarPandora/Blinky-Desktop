...
import com.blinky.istesa.Model.Warek;
...
public class FormWarek{
    private static Stage window;
    private static Home home;
    private static BorderPane border = new BorderPane();
    private static Scene scene = new Scene(border, 1280, 720);
    private static Warek data;

    private static Input[] formInputList = new Input[]{
        new Input("nmWarek", new Label("Nama Warek"), new TextField(), 1, "Masukkan nama warek").setType("TextField"),
        new Input("uWarek", new Label("Username Warek"), new TextField(), 2, "Masukkan username warek").setType("TextField"),
        new Input("pwWarek", new Label("Password Warek"), new PasswordField(), 3, "Masukkan password warek").setType("TextField"),
        new Input("nidn", new Label("NIDN Dosen"), new TextField(), 5, "Masukkan NIDN Dosen").setType("TextField"),
    };

    private static List<Input> readyInput = new ArrayList<Input>();

    public FormWarek(Stage rStage, Home h, String type){
        home = h;
    ...
    public FormWarek(Stage rStage, Home h, String type, Warek obj){
        home = h;
        data = obj;
        ...
        border.setPadding(
            new Insets(
                (window.getHeight() / 2) - (type.equals("edit") ? 300 : 150), 
                (window.getWidth() / 2) - 250, 
                (window.getHeight() / 2) - (type.equals("edit") ? 300 : 150), 
                (window.getWidth() / 2) - 250
            )
    ...
    public VBox createFormAdd(){
        ...
        Text formTitle = new Text("Tambah Warek");
        
        Button btnAcc = new Button("Tambahkan");
        btnAcc.setMaxWidth(Double.MAX_VALUE);
        btnAcc.setOnAction(e -> {
            Warek warek = new Warek();
            warek.setNmWarek(getInputValue("nmWarek"));
            warek.setUWarek(getInputValue("uWarek"));
            warek.setPwWarek(getInputValue("pwWarek"));
            warek.setNidn(getInputValue("nidn"));
    ...
    public VBox createFormEdit(){
        ...
        Text formTitle = new Text("Edit Warek");
        
        Button btnAcc = new Button("Perbarui");
        btnAcc.setMaxWidth(Double.MAX_VALUE);
        btnAcc.setOnAction(e -> {
            data.setNmWarek(getInputValue("nmWarek"));
            data.setUWarek(getInputValue("uWarek"));
            data.setPwWarek(getInputValue("pwWarek"));
    ...
    public VBox createFormDelete(){
        ...
        Text formTitle = new Text("Hapus Warek");
        Text formSubtitle = new Text("Apakah anda yakin ingin menghapus warek '" + data.getNmWarek() + "' ?");
